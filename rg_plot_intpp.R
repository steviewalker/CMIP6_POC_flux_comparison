...
plot_intpp <- function(model.name) {
  
  
    change = raster(paste0("~/spatial_analysis/raster_output/NPP/",model.name,"_intpp_change_rg.asc"))
    his = raster(paste0("~/spatial_analysis/raster_output/NPP/",model.name,"_intpp_his_rg.asc"))
    lt = raster(paste0("~/spatial_analysis/raster_output/NPP/",model.name,"_intpp_lt_rg.asc"))
    ...
    if(model.name == "Multi-model") {
      ...
      ggsave(paste(model.name,"_intpp_his.png",sep=""), plot = fig_his, path = "~/spatial_analysis/figures/multimodel/", width = 28, height = 15, units = "cm", dpi = 400)
      ggsave(paste(model.name,"_intpp_lt.png",sep=""), plot = fig_lt, path = "~/spatial_analysis/figures/multimodel/", width = 28, height = 15, units = "cm", dpi = 400)
      ggsave(paste(model.name,"_intpp_change.png",sep=""), plot = fig_change, path = "~/spatial_analysis/figures/multimodel/", width = 28, height = 15, units = "cm", dpi = 400)
      
    } else {
      model.name = model.name
    }
    
    ggsave(paste(model.name,"_intpp_map.png",sep=""), plot = figure, path = "~/spatial_analysis/figures/NPP/", width = 22, height = 30, units = "cm", dpi = 400)
